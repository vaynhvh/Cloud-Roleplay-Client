let storage = {
    ...
    radioSettingPushToTalk: false,
    radioSettingFullsend: false,
...
function OnPageChange(currentPage, newPage) {
    ...
    switch (newPage) {
        ...
        case "radio":
            ...
            break;
        case "dispatch":
            LoadDispatchReceiversFromServer();
            break;
        default:
            break;
    }
        ...
        if (data.currentSelectedReciever == reciever || data.currentSelectedReciever == sender) {
            ...
            switch (type) {
                case 1:
                    MeMessage = `
                    <div class="messages-group my">
                        <div class="message">${content.replace(/(<([^>]+)>)/ig, "")}<p>${time} - ${date}</p></div>
                    </div>`;
                    YouMessage = `
                    <div class="messages-group">
                        <div class="message">${content.replace(/(<([^>]+)>)/ig, "")}<p>${time} - ${date}</p></div>
...
async function ChangeToPushToTalk(state) {
    RemoteCall("Phone:Radio:ChangePushToTalk", state)
}

async function ChangeToFullSend(state) {
    RemoteCall("Phone:Radio:ChangeFullSend", state)
}
...
function AddLifeinvaderMessage(id, playerId, content, phonenumber, date, type) {

    if (storage.lifeinvader_messages.find(x => x.id == id)) return;

    storage.lifeinvader_messages.push(new LifeinvaderModel(id, playerId, content, phonenumber, date, type));
    ...
    <div id="lifeinvader-message-${id}" class="lifeinvader-message-wrapper">
        ...
        <div class="footer">
            <i class="fa-regular fa-comment" onclick="TryOpenMessangerChat(${phonenumber})"></i>
            <i class="fa-regular fa-phone-flip" onclick="RemoteCall('Phone:Call:Try', ${phonenumber})"></i>
            <p>Tel: ${phonenumber}</p>
            <p>${date}</p>
        </div>
    </div>
...
const RemovePlayerLifeinvaderMessages = (playerId) => {
    while(storage.lifeinvader_messages.find(x => x.playerId == playerId)) {
        let index = storage.lifeinvader_messages.findIndex(x => x.playerId == playerId);
        let item = storage.lifeinvader_messages.find(x => x.playerId == playerId);

        if (index == -1 || item == null) {
            return;
        }

        storage.lifeinvader_messages.splice(index, 1);
        $(`#lifeinvader-message-${item.id}`).remove();
    }
};
...
function InitPhone(smartphoneObject) {
    ...
    storage.settings = smartphone.settings;
    storage.gps_locations = smartphone.gps_locations;
...
/* DISPATCHES */
let receivers = [];
let currentFactionId = 1;

const LoadDispatchReceiversFromServer = () => {
    RemoteCall("Server:Phone:LoadDispatchReceivers");
};
...
const LoadReceivers = (data) => {
    ...
    data.forEach(element => {
        if (receivers.find(x => x.id == element.id)) return;
        ...
        $('#dispatch .container .workspace .rows').append(`
            <div class="profile" data-factionId="${element.id}" onclick="SwitchDispatch(${element.id})">
                <div class="factionImage">
                    <img src="../utils/img/fractionLogos/${element.id}.png" alt="faction-logo" />
                </div>
...
const SwitchDispatch = (id) => {
    currentFactionId = id;

    $('#dispatch').css('display', 'none');
    $('#sendDispatch').css('display', 'block');

    let item = receivers.find(x => x.id === id);
    if (item === null) {
        return;
    }

    $('#sendDispatch .container .workspace .header').append(`
         <div class="image">
            <img src="../utils/img/fractionLogos/${item.id}.png" alt="faction-logo" />
         </div>
         <div class="title">
            ${item.name}
         </div>
...
const SendDispatch = () => {
    if (currentFactionId >= 1) {
        let message = $('#dispatchMessage').val();
        if (message.length <= 0) return;

        mp.trigger("Client:Phone:SendDispatch", currentFactionId, message);
    }
};

const CancelDispatch = () => {
    if (currentFactionId >= 1) {
        mp.trigger("Client:Phone:CancelDispatch", currentFactionId);
    }
};

const CloseDispatch = (playerId) => {
    if (storage.dispatches.find(x => x.playerId === playerId)) {
        let index = storage.dispatches.findIndex(x => x.playerId === playerId);

        storage.dispatches.splice(index, 1);
    }
};

const AddDispatch = (id, playerId, faction, message) => {
    if (!storage.dispatches.find(x => x.playerId === playerId)) {
        storage.dispatches.push(new DispatchModel(id, playerId, faction, message));
    }
};
...
const LoadAllNotes = () => {
    RemoteCall("Server:Phone:LoadAllNotes");
};
...
const LoadNotes = (data) => {
    ...
    $('#addNotes .container .workspace .body .title #noteTitle').val(``)
    $('#addNotes .container .workspace .body .content #noteMessage').val(``)

    data.forEach(element => {
        if (!notes.find(x => x.id === element.id)) {
            notes.push({
                id: element.id,
                title: element.title,
            ...
            if (element.id > 1) {
                $('.notes-list').append(`
                    ...
                    <div class="notes-item">
                        <div class="notes-item-info" onclick="OpenNote(${element.id})">
                        ...
                        <div class="notes-item-control">
                            <i class="fa-solid fa-trash" onclick="DeleteNote(${element.id})"></i>
                        </div>
                    </div>
                `);
            } else {
    ...
    if (count === 1) $('#notesCount').text(`${count} Notiz`)
    else if (count > 1) $('#notesCount').text(`${count} Notizen`)
};

const OpenNote = (noteId) => {
    if (notes.find(x => x.id === noteId)) {
        let item = notes.find(x => x.id === noteId);
        if (item == null) return;

        $('#addNotes').css('display', 'block')
        $('#notes').css('display', 'none')

        $('#addNotes .container .workspace .body .title #noteTitle').val(`${item.title}`)
        $('#addNotes .container .workspace .body .content #noteMessage').val(`${item.content}`)
...
const ReloadNotes = () => {
    ...
    notes.forEach(element => {
        if (notes.findIndex(x => x.id === element.id) >= 1) {
            $('.notes-list').append(`
                    <hr>
                    <div class="notes-item">
                        <div class="notes-item-info" onclick="OpenNote(${element.id})">
...
const UpdateNotes = () => {
    if (notes.find(x => x.id === currentNoteId)) {
        let itemIndex = notes.findIndex(x => x.id === currentNoteId);
        if (itemIndex == null) return;

        notes.splice(itemIndex, 1);
        notes.push({
            id: currentNoteId,
            title: $('#addNotes .container .workspace .body .title #noteTitle').val(),
            content: $('#addNotes .container .workspace .body .content #noteMessage').val(),
            date: `${new Date().getDay()}.${new Date().getMonth()}.${new Date().getFullYear()}`
        });

        $('#addNotes').css('display', 'none')
...
const DeleteNote = (nodeId) => {
    if (notes.find(x => x.id === nodeId)) {
        let itemIndex = notes.findIndex(x => x.id === nodeId);
        if (itemIndex == null) return;
