...
mp.events.add("Client:Phone:RemovePlayerLifeinvaders", (playerId) => {
    if (phone != null) {
        phone.execute(`RemovePlayerLifeinvaderMessages(${playerId});`);
    }
});
...
// DISPATCHES
mp.events.add({
    "Client:Phone:LoadReceivers": (data) => {
        if (phone == null) return;

        phone.execute(`LoadReceivers('${data}')`);
    },
    "Client:Phone:SendDispatch": (factionId, message) => {
        if (phone == null) return;

        mp.events.callRemote("Server:Phone:SendDispatch", factionId, message);
    },
    "Client:Phone:AddDispatch": (id, playerId, faction, message) => {
        if (phone == null) return;

        phone.execute(`AddDispatch(${id}, ${playerId}, ${faction}, ${message});`);
    },
    "Client:Phone:CancelDispatch": (factionId) => {
        if (phone == null) return;

        mp.events.callRemote("Server:Phone:CancelDispatch", factionId);
    },
    "Client:Phone:CloseDispatch": (playerId) => {
        if (phone == null) return;

        phone.execute(`CloseDispatch(${playerId})`);
    }
});

// NOTES
mp.events.add({
    "Client:Phone:LoadNotes": (data) => {
        if (phone === null) return;

        phone.execute(`LoadNotes('${data}')`);
    },
    "Client:Phone:UpdateNote": (id, title, content, date) => {
        if (phone === null) return;

        mp.events.callRemote("Server:Phone:UpdateNote", id, title, content, date);
    },
    "Client:Phone:CreateNote": (title, content, date) => {
        if (phone === null) return;

        mp.events.callRemote("Server:Phone:CreateNote", title, content, date);
    },
    "Client:Phone:DeleteNote": (id) => {
        if (phone === null) return;

        mp.events.callRemote("Server:Phone:DeleteNote", id);
    }
});
