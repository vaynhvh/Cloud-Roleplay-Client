    ...
    async loadAll() {
        await this.resetAll().then(() => {
            ...
            for (const x of this.Containers) {
                ...
                if (x.ContainerID == 3)
                    document.getElementById('weaponscontainerdings').style.display = 'none'
...
async function elementItemOnClick(element) {
    ...
    if (slot.Item != null) {
        ...
        currContainerId = $(element).attr('data-containerid')
    }
}
let targetId = -1   
...
function CloseMenu() {
    mp.trigger('Client:PlayerSearch:Hide')
}

async function tryTakeItem() {
    ...
    if (typeof (value) == 'number' && value >= 1) {
        mp.trigger('Client:PlayerSearch:TakeItem', targetId, currContainerId, memory.LastDraggedItem.OldSlot.SlotID, value);
        memory.resetSelectedItem();
    }
}
async function tryThrowItem() {
    ...
    if (typeof (value) == 'number' && value >= 1) {
        mp.trigger('Client:PlayerSearch:ThrowItem', targetId, currContainerId, memory.LastDraggedItem.OldSlot.SlotID, value)
    }
}
